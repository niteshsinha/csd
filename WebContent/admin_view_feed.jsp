...
<title>Customer Service Desk : View Feedback</title>
...
//For Paging of the Feedbacks....
int Feed_page_num = 0;
try {
	Feed_page_num = Integer.parseInt(request.getParameter("Feed_page"));
} catch (Exception e)
{
	Feed_page_num = 1;
}
int Feed_rec_per_page = 8;
int Feed_start_limit = (Feed_page_num-1)*Feed_rec_per_page + 1;
int Feed_stop_limit = (Feed_page_num)*Feed_rec_per_page;
...
%>
  <tr>
    ...
    <td align="center"><a href="admin_detail_feed.jsp?FeedCompNo=<%=comp.getComplainNo()%>">View Details</a></td>
  </tr>
   <%
	}
	%>
...
<table width="100%">
	<tr>
		<td width="50%" align="left"><% if(Feed_page_num != 1) { %><a href="?Feed_page=<%=(Feed_page_num-1)%>">Prev</a><% } %>&nbsp;</td>
		<td width="50%" align="right"><% if(Feed_rec_count != 0) { %><a href="?Feed_page=<%=(Feed_page_num+1)%>">Next</a><% } %>&nbsp;</td>
	</tr>
</table>
